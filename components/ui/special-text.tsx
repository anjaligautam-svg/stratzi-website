...
/**
 * SpecialText — character-scramble reveal animation.
 *
 * Adapted from the source component:
 *  - Import path swapped from `motion/react` → `framer-motion` (we already
 *    have framer-motion installed; both export `useInView` with the same
 *    signature, so no `motion` package install needed).
 *  - Stripped the hardcoded `font-mono font-medium h-4.5 leading-5` so the
 *    final settled text fully inherits the parent's typography (size, font,
 *    color). Critical for placing this inside a `display-xl` headline.
 *  - `className` is the only styling source — pass whatever Tailwind/CSS
 *    classes you want and they apply throughout the scramble + settled
 *    states.
 */
