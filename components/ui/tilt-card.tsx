...
export interface TiltCardProps {
  ...
  spotlight?: boolean;
  /**
   * Spotlight gradient color. Defaults to a soft primary-teal glow
   * (`var(--color-primary-soft)` at ~38% alpha) so the highlight feels
   * brand-aligned. Pass any CSS color string to override.
   */
...
/**
 * TiltCard — 3D-tilt wrapper with an optional cursor-following spotlight.
 * Use it as the outer wrapper for any card you want to feel interactive.
 *
 * Defaults are tuned restrained for editorial feel (subtle 8° tilt, 1.02 scale).
 * Adjust `tiltLimit` / `scale` / `effect` per usage.
 */
export function TiltCard({
  ...
  effect = "evade",
  spotlight = true,
  spotlightColor = "rgba(176, 238, 237, 0.45)",
  className,
  style,
  children,
}: TiltCardProps) {
